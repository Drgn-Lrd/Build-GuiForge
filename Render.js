  ...
  if (def.isTabControl) {
    ...
    state.controls
      ...
      .forEach(ch => {
        ...
        // Preview-only label swap - mirrors wizardShowFunctionLines'
        // Run/Finish runtime logic (Wizard-Builder.js) without touching the
        // button's own stored Text, which stays the generic "Next" default.
        if (ch.wizardRole === 'next') {
          const label = wizardNextButtonLabelForActivePage(c);
          const btnFace = childEl.querySelector('.rc-button');
          if (btnFace) btnFace.textContent = label;
        }
