/*
    ...
    1. cliArgAssignmentLines call site now passes the injection's actual
    target event name (Cli-Preview-Builder.js 1.4), so orList can tell
    whether it's running inside the tagged CheckedListBox's own
    ItemCheck (needs the $e.Index/$e.NewValue staleness workaround) or
    somewhere else (e.g. a gate's CheckedChanged, where CheckedItems is
    already accurate).
*/

const CODEGEN_WINFORMS_VERSION = '1.20';
      ...
      Object.entries(eventsForCodegen).forEach(([evtName, data]) => {
        ...
        if (cliEntries && cliEntries.length) {
          const cliLines = cliEntries
            .map(e => cliArgAssignmentLines(e.sourceCtrl, e.cli, e.key, e.previewVar, evtName))
            .join('\n')
            .split('\n').join('\n    ');
          body = body ? `${body}\n    ${cliLines}` : cliLines;
        }
