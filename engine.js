/*
    ...
    Revision: started on the event system (Group 1 from the priority
    list). (1) Added EVENT_HELP - a per-event-NAME guidance dictionary
    (Click, TextChanged, ValueChanged, etc, shared across every control
    type since an event means the same thing everywhere it appears),
    surfaced via a new (i) info button in each event's header, opening
    the Info modal on click. (2) Added usage explanations to every
    EVENT_SNIPPETS entry, surfaced via a matching (i) button next to the
    snippet dropdown. (3) Multiple actions per event: replaced the single
    Code textarea with buildActionsEditor - a list of individually
    editable/removable action cards ("+ Add action"), each with its own
    snippet dropdown/insert/info button. Actions are stored joined by
    blank lines in the same data.code string codegen has always read, so
    this is purely a friendlier editing view - no codegen consumer needed
    to change. Directly answers "can multiple things happen on one event"
    - yes, and now there's a real UI for building that up action by
    action instead of one undifferentiated blob of code.
*/

const ENGINE_VERSION = '1.13';
...
const EVENT_SNIPPETS = [
  ...
  { label: 'Close the form', code: `$Form.Close()`, help: 'Closes the whole window. Common on a "Cancel" or "Exit" button, or after a "Save and close" action completes.' },
...
document.addEventListener('DOMContentLoaded', initEngine);// non-separator item ships with real default code (editable per-item),
// not just a label - so File > Exit, Help > About, etc. actually do
// something out of the box instead of being empty stubs.
const PRESET_MENU_DEFAULT = [
  {
